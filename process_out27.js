...
(async () => {
  const already = rawDb.prepare('SELECT tx_hash FROM processed_withdrawals WHERE withdraw_nonce=? AND dest_chain_id=8453').get('27');
  if (already) { console.log('Already done:', already.tx_hash); return; }

  const taoPrice = await getTaoPrice();
  const netTao = 0.035 * 0.95;
  const usdcAmt = ethers.parseUnits((netTao * taoPrice).toFixed(6), 6);
  const recip = ethers.getAddress('0x5eb28541a5b6b9c5c25a34905aa97f80dd63dd53');
  ...
  const tx = await vault.release(USDC, recip, usdcAmt, 27);
  console.log('TX:', tx.hash);
  await tx.wait();
  rawDb.prepare('INSERT OR IGNORE INTO processed_withdrawals (withdraw_nonce, dest_chain_id, tx_hash) VALUES (?,8453,?)').run('27', tx.hash);
  console.log('Done ✓');
})().catch(console.error);
