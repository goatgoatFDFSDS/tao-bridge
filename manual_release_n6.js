require('dotenv').config();
const { ethers } = require('ethers');

const provider = new ethers.JsonRpcProvider('https://mainnet.base.org');
const signer   = new ethers.Wallet(process.env.RELAYER_PRIVATE_KEY, provider);

const VAULT = '0xcd3BB66c4baD58c9C0283C70CbCBD7950Aa4c063';
const USDC  = '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913';
const TO    = '0x47CDDFBc26Da2dF3b10F360b32c52A33e309023d';
const AMOUNT = 53390000n; // 53.39 USDC (6 dec)

const VAULT_ABI = [
  'function removeLiquidity(address token, uint256 amount) external',
];
const ERC20_ABI = [
  'function transfer(address to, uint256 amount) external returns (bool)',
  'function balanceOf(address) view returns (uint256)',
];

const vault = new ethers.Contract(VAULT, VAULT_ABI, signer);
const usdc  = new ethers.Contract(USDC,  ERC20_ABI, signer);

(async () => {
  // Step 1: pull USDC from vault to dev wallet
  console.log('Step 1: removeLiquidity', ethers.formatUnits(AMOUNT, 6), 'USDC from vault...');
  const tx1 = await vault.removeLiquidity(USDC, AMOUNT, { gasLimit: 120000 });
  console.log('TX1:', tx1.hash);
  await tx1.wait();
  console.log('✅ Pulled from vault');

  // Step 2: send to user
  console.log('Step 2: sending', ethers.formatUnits(AMOUNT, 6), 'USDC to', TO);
  const tx2 = await usdc.transfer(TO, AMOUNT, { gasLimit: 80000 });
  console.log('TX2:', tx2.hash);
  await tx2.wait();
  console.log('✅ Sent to user');
})().catch(console.error);
